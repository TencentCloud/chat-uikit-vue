  ...
  <div class="TUI-contact" :class="[isPC ? ' ' : 'TUI-contact-H5']">
    <aside class="TUI-contact-left">
      <header class="TUI-contact-left-header">
        <div class="search">
          ...
          <div class="search-box" v-else>
            ...
            <span class="search-cancel" @click="toggleSearch">{{ TUITranslateService.t('取消') }}</span>
          </div>
        </div>
      </header>
      <ul class="TUI-contact-column" v-if="!searchStatus">
        <li class="TUI-contact-column-item">
          <header @click="openContactList('group')">
              <Icon :file="listName === 'group' ? downIcon : rightIcon" :width="'16px'" :height="'16px'"></Icon>
            <main>
          ...
          <ul class="TUI-contact-list"  v-if="listName === 'group'">
            ...
            >
              <aside class="left">
                <Icon :file="item.avatar || 'https://web.sdk.qcloud.com/component/TUIKit/assets/group_avatar.png'" :width="'36px'" :height="'36px'"></Icon>
              </aside>
              <main class="content">
                <ul>
        ...
        <li class="TUI-contact-column-item">
          <header @click="openContactList('friend')">
            <Icon :file="listName === 'friend' ? downIcon : rightIcon" :width="'16px'" :height="'16px'"></Icon>
            <main>
              <label>{{ TUITranslateService.t('TUIContact.我的好友') }}</label>
            </main>
          </header>
          <ul class="TUI-contact-list" v-if="listName === 'friend'">
            <li class="TUI-contact-list-item" :class="[currentFriend.userID === item.userID && 'selected']"
              v-for="(item, index) in myFriendList" :key="index" @click="clickListItem(item)">
              <aside class="left">
                <Icon :file="item.avatar || 'https://web.sdk.qcloud.com/component/TUIKit/assets/avatar_21.png'" :width="'36px'" :height="'36px'"></Icon>
              </aside>
              <main class="content">
                <ul>
      ...
      <ul class="TUI-contact-list" v-else>
          ...
          >
            <aside class="left">
              <Icon v-if="isUniFrameWork" :file="searchGroup.avatar || 'https://web.sdk.qcloud.com/component/TUIKit/assets/group_avatar.png'" :width="'36px'" :height="'36px'"></Icon>
              <img
                v-else
                class="avatar"
                :src="searchGroup.avatar || 'https://web.sdk.qcloud.com/component/TUIKit/assets/group_avatar.png'"
    ...
    <main :class="['TUI-contact-main',isUniFrameWork && !isWeChat &&'TUI-contact-main-uni']" v-if="currentGroup.groupID || currentFriend.userID">
      <header class="TUI-contact-main-h5-title" v-if="!isPC" @click="clearData">
          <Icon :file="backSVG"></Icon>
          <h1>{{ currentGroup.name || TUITranslateService.t('TUIContact.我的好友')}}</h1>
      </header>
      <div v-if="currentGroup.groupID" class="TUI-contact-main-info">
        <header class="TUI-contact-main-info-header">
          ...
          <Icon :file="currentGroup.avatar || 'https://web.sdk.qcloud.com/component/TUIKit/assets/group_avatar.png'" :width="'5rem'" :height="'5rem'"></Icon>
        </header>
        <main class="TUI-contact-main-info-main" v-if="isNeedPermission">
          <label>{{ TUITranslateService.t('TUIContact.请填写验证信息') }}</label>
          <textarea v-model="currentGroup.applyMessage" :disabled="currentGroup.apply"></textarea>
        </main>
        <footer class="TUI-contact-main-info-footer">
          <p v-if="currentGroup.apply && currentGroup.type === 'AVChatRoom'">{{ TUITranslateService.t('TUIContact.已加入') }}</p>
          <p v-else-if="currentGroup.apply">{{ TUITranslateService.t('TUIContact.已申请') }}</p>
          <button class="btn btn-default" v-else-if="isNeedPermission" @click="joinGroup(currentGroup)">
            {{ TUITranslateService.t('TUIContact.申请加入') }}
          </button>
          <button class="btn btn-default" v-else-if="!currentGroup.selfInfo.userID" @click="joinGroup(currentGroup)">
            {{ TUITranslateService.t('TUIContact.加入群聊') }}
          </button>
          <button class="btn btn-cancel" v-else-if="currentGroup.selfInfo.userID &&
          ...
          <button class="btn btn-cancel" v-else @click="quitGroup(currentGroup)">{{ TUITranslateService.t('TUIContact.退出群聊') }}</button>
          <button v-if="currentGroup.selfInfo.userID" class="btn btn-default"
            @click="enterConversation(currentGroup.groupID, 'GROUP')">
            {{ TUITranslateService.t('TUIContact.进入群聊') }}
          </button>
        </footer>
      </div>
      <div v-else-if="currentFriend.userID && listName === 'friend'" class="TUI-contact-main-info">
        <header class="TUI-contact-main-info-header">
          <ul class="list">
          ...
          <Icon :file="currentFriend.avatar || 'https://web.sdk.qcloud.com/component/TUIKit/assets/avatar_21.png'" :width="'5rem'" :height="'5rem'"></Icon>
        </header>

        <footer class="TUI-contact-main-info-footer">
          <button class="btn btn-default" @click="enterConversation(currentFriend.userID, 'C2C')">
...
const isNeedPermission = computed(() => {
   return (currentGroup.value as any).joinOption === TUIChatEngine.TYPES.JOIN_OPTIONS_NEED_PERMISSION;
});
